...
from .schemas.request_schemas import ProductCategoryCreate, ProductCategoryUpdate, ProductUpdate, ProductCreate
from .schemas.response_schemas import ProductCategoryResponse, ProductResponse, ProductUpdateResponse
...
def update_product_category(category_id: int, category_update: ProductCategoryUpdate, db: Session = Depends(get_db)):
    ...
    if category is None:
        raise HTTPException(status_code=404, detail=common_message.detail)
    return category
...
def update_product(product_id: int, product_update: ProductUpdate, db: Session = Depends(get_db)):
    ...
    if product is None:
        raise HTTPException(status_code=404, detail=common_message.update_detail)
    return product
...
def delete_product_item(product_id: int, db: Session = Depends(get_db)):
    ...
    if product is None:
        raise HTTPException(status_code=404, detail=common_message.search_detail)
...
def product_search_by_filter(
    ...
    if category_id <= 0:
        raise HTTPException(status_code=400, detail=common_message.id_missing_detail)
